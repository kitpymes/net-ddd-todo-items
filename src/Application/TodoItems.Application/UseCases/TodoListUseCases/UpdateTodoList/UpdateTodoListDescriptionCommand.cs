...
public record UpdateTodoListDescriptionCommand(Guid TodoListId, string Description) : IRequest<IAppResult>;

public class UpdateTodoListDescriptionCommandHandler(ITodoListRepository repository) : IRequestHandler<UpdateTodoListDescriptionCommand, IAppResult>
{
    private readonly ITodoListRepository _repository = repository;

    public async Task<IAppResult> Handle(UpdateTodoListDescriptionCommand request, CancellationToken cancellationToken)
    {
